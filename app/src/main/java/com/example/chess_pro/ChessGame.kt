...
object ChessGame {
    ...
    private fun canPawnMove(from: Square, to: Square): Boolean {
        ...
        if (from.col == to.col) {
            if (from.row + direction == to.row && pieceAt(to) == null) {
                return true
            } else if (from.row + 2 * direction == to.row && from.row == (if (player == Player.WHITE) 1 else 6) && pieceAt(to) == null && pieceAt(Square(from.col, from.row + direction)) == null) {
                return true
            }
        } else if (abs(from.col - to.col) == 1 && from.row + direction == to.row) {
            pieceAt(to)?.let {
                if (it.player != player) {
                    return true
                }
